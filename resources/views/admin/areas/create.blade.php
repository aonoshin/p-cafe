...
@section('admin-title', 'エリア登録')

@section('main')
<ul class="back-admin-top">
    <li><a href="{{route('admin.top')}}">管理画面トップへ</a></li>
    <li><a href="{{route('admin.areas.index')}}">エリア一覧へ</a></li>
</ul>
<div class="admin-box-form-create">
    <form method="post" action="{{route('admin.areas.store')}}" enctype="multipart/form-data">
        ...
        <ul>
            <li>
                エリア名&nbsp;<span class="must">必須＆20文字以内</span>
                @if($errors->has('name'))
        ...
        <ul>
            <li>
                エリアURL&nbsp;<span class="must">必須</span>
                @if($errors->has('url'))
        ...
        <ul>
            <li>エリアイメージ</li>
            <li><input class="width-100" type="file" name="image" value="{{old('image')}}"></li>
        </ul>
